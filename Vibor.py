...
while True:
    ...
    mag = int(input())
    if mag == 1:
       print("Теперь ты маг - стихийный",)
    elif mag == 2:
        print("Теперь ты маг - тёмный")
    else:
        print("Теперь ты маг - светлый",)
    print()
    ...
    if way == 1 or 2:
        ...
        if way == 2:
            print("Вы проиграли.")
            print("Вы желаете [1]-начать заново или [2]-завершить игру?")
            ex = int(input())
            if ex == 1:
                continue
            else:
            ...
            print("Встал перед ним снова [РАНДОМ] (вы договоритись) - если русалка, (Вы умираете) если тварь заморская.")
            print()
            ...
            if monster == 2:
                ...
                print("Тебе повезло)) Вот он снова избавился от своей проблемы, договорившись с русалкой, да пошел он дальше, дабы с пути не сбиться.")
                print("Спустя сутки...")
                print()
                ...
                if way == 2:
                    ...
                    if monster == 1:
                        ...
                        if way == 1:
                            ...
                            print("Ты сражался с честью и выиграл эту жабу.")
                            print("[СЮЖЕТ]")
                            print("Маг продолжил свой путь. И вот, шёл он дальеше, наткнулся на сидящего на камне старика  с деревянным посохом(в общем тоже маг)")
                            ...
                            print("(Существа ландшафт, погода и т.д) с возможностью приблизить картинку. Вышел он из пещеры, да направился дальше.")
                            print()
                            ...
                            if way == 2:
                                ...
                                print("Там он сдох в старости. Конец игры вы проиграли. Выиграть не возможно)")
                                break
                        ...
                        if way == 2:
                            print("Маг высказался о нём, его народе, власти достаточно хорошо.")
                            print("[СЮЖЕТ]")
                            print("Маг продолжил свой путь. И вот, шёл он дальеше, наткнулся на сидящего на камне старика  с деревянным посохом(в общем тоже маг)")
                            print("и, кажется, ключ у этого мага. Ключ нужен нашему магу, поэтому он подошёл к старику, да стал просить его.")
                            print("А старик не простой оказался, он потребовал за ключ шкуру жабы-электро-мага-с-посохом.")
